...
def pesquisar_produtos():
    ...
    lojasBusca = DatabaseController().sqlRead(
        "SELECT regexproduto,site, URL,xpathproduto,xpathpesquisa, xpathbotaopesquisa, xpathfiltro,xpathlistapesquisa FROM loja WHERE URL IS NOT NULL AND xpathproduto IS NOT NULL AND regexproduto IS NOT NULL AND xpathpesquisa IS NOT NULL AND xpathlistapesquisa IS NOT NULL AND xpathbotaopesquisa IS NOT NULL order by site;"
    )
